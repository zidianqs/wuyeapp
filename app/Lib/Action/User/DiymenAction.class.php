...
class DiymenAction extends UserAction{
	...
	public function  class_add(){
		if(IS_POST){
			...
			$class=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>0))->order('sort desc')->select();
			$this->assign('class',$class);
			$this->display();
		}
	}
	public function  class_del(){
		$class=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>$this->_get('id')))->order('sort desc')->find();
		//echo M('Diymen_class')->getLastSql();exit;
		if($class==false){
			$back=M('Diymen_class')->where(array('token'=>$this->_get('token'),'id'=>$this->_get('id')))->delete();
			if($back==true){
				$this->success('删除成功');
			}else{
				$this->error('删除失败');
	...
	public function  class_edit(){
		if(IS_POST){
			...
			$data=M('Diymen_class')->where(array('token'=>$this->_get('token'),'id'=>$this->_get('id')))->find();
			Log::write(session('token'));
			Log::write("vip:" . session('viptime'));
			Log::write($this->_get('id'));
			if($data==false){
				$this->error('您所操作的数据对象不存在！');
			}else{
				$class=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>0))->order('sort desc')->select();//dump($class);
				$this->assign('class',$class);
	...
	public function  class_send(){
		if(IS_GET){
			$api=M('Diymen_set')->where(array('token'=>$this->_get('token')))->find();
			if($api['appid']==false||$api['appsecret']==false){$this->error('必须先填写【AppId】【 AppSecret】');exit;}
			Log::write("API:" . dump($api, false), Log::INFO);
			...
			$class=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>0))->limit(3)->order('sort desc')->select();//dump($class);
			$kcount=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>0))->limit(3)->order('sort desc')->count();
			$k=1;
			foreach($class as $key=>$vo){
				//主菜单

				$data.='{"name":"'.$vo['title'].'",';
				$c=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>$vo['id']))->limit(5)->order('sort desc')->select();
				//dump($c);
				$count=M('Diymen_class')->where(array('token'=>$this->_get('token'),'pid'=>$vo['id']))->limit(5)->order('sort desc')->count();
				//子菜单
				if($c!=false){
					$data.='"sub_button":[';
				}else{
