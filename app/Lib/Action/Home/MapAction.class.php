...
class MapAction extends Action{
	...
	public function jiejing() {
		...
		$where=array('token'=>$this->token);
		$thisCompany=$company_model->where($where)->order('isbranch ASC')->find();
		$lat = $thisCompany['latitude'];
		$lng = $thisCompany['longitude'];
		$panoId = $thisCompany['panoid'];
		if (!$panoId) {
			// 百度地图和soso地图有些差异，需要转换
			$xx = $lng - 0.0065;
			$yy = $lat - 0.006;
			$zz = sqrt($xx * $xx + $yy * $yy) - 0.00002 * sin($yy * pi());
			$theta = atan2($yy, $xx) - 0.000003 * cos($xx * pi());
			$lng = $zz * cos($theta); 
			$lat = $zz * sin($theta);
			$x = 111319.49077777778 * $lng;
			$lat = log(tan(.008726646259971648 * (90+$lat))) / .017453292519943295;
			$y = 111319.49077777778 * $lat;
			$url = "http://sv.map.qq.com/xf?x=" .$x . "&y=" . $y . "&r=500&key=d84d6d83e0e51e481e50454ccbe8986b&output=jsonp&pf=jsapi&cb=QQMapLoader.cbhtfrvf4u3";
			$re = file_get_contents($url);
			$re = str_replace('QQMapLoader.cbhtfrvf4u3&&QQMapLoader.cbhtfrvf4u3(', '', $re);
			$re = str_replace(');', '', $re);
			$i = strpos($re, 'svid');
			$re = '{"detail":{"' . substr($re, $i); //为了删除一些乱码的中文字符
			$arr = json_decode($re, true);
			$panoId = $arr['detail']['svid'];
		}
		$mobile_url = "http://jiejing.qq.com/?from=singlemessage&isappinstalled=0#pano=" .$panoId. "&heading=336&pitch=-11&nav=0&poi=1&ref=wx";
		$return=array();
		$return[] = array('街景地图', '您可以通过街景地图查看小区', 'http://www.missyuan.net/uploads/allimg/100516/162HS507-0.jpg', $mobile_url);
		return array($return,'news');
	}
